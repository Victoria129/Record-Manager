  ...
  def list_all_books
    ...
    if books.empty?
      ...
      books.each_with_index do |book, index|
        puts "#{index + 1}. Book ID:#{book.id} by #{book.author.first_name} #{book.author.last_name}, published by #{book.publisher}"
      end
    end
  end
  ...
  def list_all_albums
    ...
    if albums.empty?
      ...
      albums.each_with_index do |album, index|
        puts "#{index + 1}. Album ID:#{album.id} by #{album.author.first_name} #{album.author.last_name}, On Spotify? #{album.on_spotify}"
      end
    end
  end
  ...
  def list_all_games
    ...
    if games.empty?
      ...
      games.each_with_index do |game, index|
        puts "#{index + 1}. Game ID:#{game.id} by #{game.author.first_name} #{game.author.last_name}, Last Played :#{game.last_played.to_s}"
      end
    end
  end
